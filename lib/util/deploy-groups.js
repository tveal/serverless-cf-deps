...
module.exports = {
  createDeployGroups: (cfnDeps, stacksToDeploy = [], exclusive = false) => {
    ...
    const groupDeps = Object.entries(groups)
      .map(([groupId, array], index) => {
        const ids = flatMap(array.map(a => a.id));
        const uniqDeps = uniq(
  ...
  createDeployReport: (deployData) => {
    ...
    const msg = [
      ...
      '===',
      '',
      `Total Stacks: ${total}, Missed: ${missedIds.length}, Untracked: ${totalUntracked}`,
      '',
      'Inputs',
      `  - Exclusive: ${exclusive}`,
      '  - Stacks to deploy',
...
function filteredDependencies(deps, subset = [], exclusive = false) {
