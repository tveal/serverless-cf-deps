...
const options = {
  ...
  CF_DEPS_STACK_NAME: {
    name: 'stack-name',
    type: 'string',
    desc: 'CloudFormation stack name, default: repo (directory) name',
    menu: [depsBin,],
    reqd: false,
    nargs: 1,
    func: (cliVal, envVal) => {
      return cliVal || envVal;
    },
  },
  CF_DEPS_COMMIT_MSG: {
  ...
  CF_DEPS_EXCLUSIVE_DEPLOY: {
    name: 'exclusive-deploy', alias: 'e',
    type: 'boolean',
    desc: 'only calculate direct dependencies between provided deploy stacks; default: false',
    menu: [groupsBin],
    reqd: false,
    nargs: 0,
    func: (cliVal, envVal) => (cliVal || !!envVal),
  },
  CF_DEPS_JS_FILTER: {
    name: 'filter',
    type: 'string',
...
const toExport = {
  CURRENT_REPO: [
    origin.sync()
      ? path.basename(origin.sync()).replace('.git', '')
      : undefined,
    process.env.CF_DEPS_STACK_NAME,
